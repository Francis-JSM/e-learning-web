...
// register email validation coding.
...
email.addEventListener("keyup", (e) => {
  ...
  if (emailval.length > 0) {
    fetch("/validate/email", {
      body: JSON.stringify({ emailid: emailval }),
      method: "POST",
    })
      .then((res) => res.json())
      .then((data) => {
...
// register username validation coding.
...
const loginfeedbackusername = document.querySelector(
  ".login-key-error-username"
);
loginusernameField.addEventListener("keyup", (e) => {
  ...
  if (usernameval.length > 0) {
    fetch("/validate/login-username", {
      body: JSON.stringify({ login_username: usernameval }),
      method: "POST",
    })
      .then((res) => res.json())
      .then((data) => {
